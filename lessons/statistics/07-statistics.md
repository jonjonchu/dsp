...
>> Bayes Law:
    ...
    P(A|B) = P(B|A) * P(A) / P(B)
>> P(A or B), both A & B being disjoint events:

    P(A or B) = P(A) + P(B)

>> Here P(A) = identical twin % = 1/300

>> P(B) = twin = P(ident) + P(frat) = 1/125 + 1/300

>> P(B|A) = prob of one being a twin, given one is an identical twin = 100%

>> P(A|B) = probability of being/having an identical twin, given one is a twin

>> __Thus,__

>> P(ident|twin) = 100%  * 1/300 / (1/125 + 1/300) = 10/22

>> __Elvis had a 10/22 chance of being an identical twin__
...
>> Frequentist statistics assign probabilities only to repeatable random events like coin flips. There are no probabilities assigned to hypothese or 'fixed but unknown values.' Bayesian statistics assigns probabilities based on degree of belief and degree of logical support. Importantly, Bayesian statistics also combines a priori assumptions into those probabilities.
